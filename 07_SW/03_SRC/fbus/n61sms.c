...
char n61_msgqueue[1][161];
char n61_msgqueue2[1][17];
...
void n61_addmsg(char *dest, char *msg)
{
	...
	strcpy(n61_msgqueue, msg);
	strcpy(n61_msgqueue2, dest);
...
int n61_smsrecv(char *dest, char *msg)
{
	...
	strcpy(msg, n61_msgqueue);
	strcpy(dest, n61_msgqueue2);
...
{
	static int n61_atbyte = 0;
	static int n61_datp = 0;
	...
	while(n61_serial_isdata()) 
	{
		...
		switch(n61_mode) 
		{
			...
			case N61MODE_DATA:
				...
				if(n > 255) 
				{
					/* so much data - error - start from the beginning */
					n61_multiple = 0;
				...
				if(n >= n61_readsize - 1) 
				{
					...
					if(n61_tmp.cs[0] == n61_tmp.cs[1] && n61_tmp.cs[0] == 0) 
					{
						...
						if(n61_tmp.type != 0x7f) 
						{
							...
							/* check if it was last sequence (data package)*/
							if(n61_tmp.size > 1 && n61_tmp.dat[n61_tmp.size-2] != 0x01) 
							{
								/* It was not last package  */
...
void n61_procmsg(struct N61_MESSAGE *msg)
{
	...
	// check if this is a msg of interest (received ACK)
	subtype = 0;
	if(msg->type != 0x7f) 
	{
		if(msg->type == 0x02) 
