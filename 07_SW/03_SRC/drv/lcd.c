...
volatile const PROGMEM unsigned char LcdCustomChar[16][8] =
{
	//0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07, 0x08, // 0. 0/5 full progress block
...
//	lcd_command(1<<LCD_CGRAM | 0x00);
	...
	lcd_command(1<<3 | 1<<LCD_ON_DISPLAY );
	// move cursor to home
	lcd_command(1<<LCD_HOME);
...
void lcd_LoadCustomChar(const PROGMEM uint8_t *lcdCustomCharArray, uint8_t romCharNum)
{
		/*
	...
	//saveDDRAMAddr = lcd_read(0) & 0x7F;

	romCharNum = (romCharNum<<3);	// each character occupies 8 bytes
	
    lcd_command((1 << LCD_CGRAM) | romCharNum);  /* set CG RAM start address 0 */
	for(i=0; i < 8  ; i++)
	{
		lcd_data(pgm_read_byte_near(&lcdCustomCharArray[i]));
	}
	// restore the previous cursor position
	//lcd_command(1<<LCD_DDRAM | saveDDRAMAddr);
...
void lcd_ProgressBar(uint16_t progress, uint16_t maxprogress, uint8_t length)
{
	uint8_t i;
